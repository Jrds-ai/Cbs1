...
import { ArrowLeft, Loader2, Check, X, AlertCircle, ArrowRight, MessageSquare } from 'lucide-react';
...
export default function ReviewPage() {
    ...
    const handleAction = async (status: 'approved' | 'rejected') => {
        ...
        try {
            ...
            // Check if this is the absolute last page being reviewed overall
            const newlyPending = allPages.filter(p => p.status === 'pending_review');
            const rejectedCount = allPages.filter(p => p.status === 'rejected').length;
            const updates: any = { generatedPages: allPages };

            // If no more pending pages, and no rejected pages -> book is Completed!
    ...
    const currentPage = pendingPages[currentIndex];
    const proxyUrl = currentPage.url.includes('googleapis.com') ? `/api/image-proxy?url=${encodeURIComponent(currentPage.url)}` : currentPage.url;
                        ...
                        {!isRejecting ? (
                            ...
                            <div className="animate-in fade-in slide-in-from-bottom-4">
                                ...
                                <textarea
                                    ...
                                    placeholder="e.g. The character's face looks distorted, or please remove the background elements..."
                                    rows={4}
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        disabled={saving}
                                        className="flex-1 py-3 bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300 font-bold rounded-xl hover:bg-slate-200 transition-colors"
                                    >
                                        Cancel
                                    </button>
                                    <button
